/*
  ...
  - You are about to drop the `testAndMeassurements` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the column `line` on the `Items` table. All the data in the column will be lost.
  - Added the required column `line_id` to the `Items` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "DayPartTest" (
    ...
    CONSTRAINT "DayPartTest_dayPart_id_fkey" FOREIGN KEY ("dayPart_id") REFERENCES "DayPart" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
